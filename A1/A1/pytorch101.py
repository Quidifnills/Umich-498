...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if remainder == 0:
        ...
        first_multiple = start + (10 - remainder)

    if first_multiple > stop:
        x = torch.empty(0, dtype=torch.float64)

    else:
        x = torch.arange(start=first_multiple,end=stop + 1,step=10,dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    last_row = x[-1, :]
    third_col = x[:, 2]
    first_two_rows_three_cols = x[1:3, 1:4]
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, col_min_idx = x.min(dim = 1)
    y = x.clone()
    y[torch.arange(x.shape[0]), col_min_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    square = ((x - mu) ** 2).sum(dim=0) / (M - 1)
    sigma = torch.sqrt(square)
    y = (x - mu) / sigma
